...
.DESCRIPTION
特定の名前と場所が指定された Microsoft Azure Web サイトを作成します。この関数は Azure モジュールで New-AzureWebsite コマンドレットを呼び出します。サブスクリプションに名前が指定された Web サイトがない場合、この関数は Web サイトを作成し、Web サイト オブジェクトを返します。このような Web サイトがある場合は、既存の Web サイトを返します。
